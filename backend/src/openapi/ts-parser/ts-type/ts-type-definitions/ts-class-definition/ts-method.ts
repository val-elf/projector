...
export class TsMethod extends TsEntity implements IOpenApiSerializable {
    private _returnType!: ITsType;

    public get retrunType(): ITsType {
    ...
    constructor(
        ...
        const methodProperties = TsMethod.parseMethodSignature(signature);
        const { methodParameters, returnType } = methodProperties;
        this.methodParameters = methodParameters;
        this._returnType = new TsType(returnType);
    ...
    private prepareResponses(responses: ITagResponse, oaSecurity: IPathSecurityDefinition[], gatherer: IOpenApiGather): IResponsesDefinition {
        ...
        if (fullDefinitions['200']) {
            fullDefinitions['200'].type = this._returnType.toOpenApi(gatherer, true);
        }
