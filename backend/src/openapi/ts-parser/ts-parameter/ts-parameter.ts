...
import { ITsParameters, TsEntity } from '../model';
import { ITsParser, ETsEntityTypes } from '../ts-readers/model';

import { TsParameterParser } from './ts-parameters-parser';
...
export class TsParameter extends TsEntity {
    ...
    public static readParameters(params: string, oreader: ITsParser): ITsParameters[] {
        params = oreader.restoreCode(params);
        const result = [];
        if (!params) return [];
        const reader = new TsParameterParser(params);
        let param = '';
        while(true) {
