...
class DuplexDataReader extends Transform {
	...
	getData() {
		return new Promise((resolve) => {
			...
			this.on('end', () => {
				resolve({ content: Buffer.concat(result), header: this.vm.multipart });
			});
		});
	}
}
...
export class Request {
	...
	_streamReceive(req) {
		...
		if(this.boundary){
			var _chunk = 0,
				finBoundary = '\r\n--' + this.boundary + '--\r\n'
			;
			this.body = new DuplexDataReader({
				transform: function(chunk, encoding, callback){
					if(!_chunk) {
						chunk = vm._processFirstBoundaryChunk(chunk);
	...
	_basicReceive(req: ERequest, ctype) {
		var data = [],
			sum = 0,
			hlength = 0,
			cl = 0
...
function collectMeta(tpl, args, id?: string) {
	var sub, res;
