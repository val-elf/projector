(function(){
	...
	angular.module('projector.directives')
		.directive('pjCathegorizedDocuments', ProjectorCathegorizedDocuments)
		.controller('ProjectorCathegorizedDocumentsController', ProjectorCathegorizedDocumentsController)
	;

	function ProjectorCathegorizedDocuments(){
		return {
			restrict: 'E',
			templateUrl: 'common/directives/widgets/projector-cathegorized-documents.html',
			scope: {
				owner: '=',
				rootAsOwner: '='
			},
			controller: 'ProjectorCathegorizedDocumentsController',
			controllerAs: 'ctrl',
	...
	function ProjectorCathegorizedDocumentsController($scope, $q, $compile, AbstractOwnerService, CathegoriesService, DocumentsService, modal, alert){
		var vm = this;
		vm.ownerService = AbstractOwnerService($scope.owner);
		vm.docService = DocumentsService($scope.owner);
		vm.owner = $scope.owner;
		vm.documents = [];
		vm.rootAsOwner = $scope.rootAsOwner !== undefined ? $scope.rootIsOwner : true;

		var catServ = CathegoriesService($scope.owner),
			openedPage = 1,
			pageCount = 80,
			needMore = true;
		...
		angular.extend(this, {			
					...
					params: {
						...
						onSave: function(cathegory){
							catServ.refresh().then(function(){
								vm.selectedCathegory = findCathegory($scope.cathegories, cathegory._id);
							});
						}
					}
				})
			...
			selectCathegory: function(){
				vm.docService = DocumentsService(vm.selectedCathegory || vm.owner);
