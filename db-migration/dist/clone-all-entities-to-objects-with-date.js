...
exports.cleanObjectsFromCreateUpdate = exports.runUpdateDbObjects = void 0;
...
function prepareSource(source) {
    const result = { $set: { _id: source._id }, $unset: {} };
    if (source._create)
        result.$set._create = source._create;
    else
        result.$unset._create = "";
    if (source._update)
        result.$set._update = source._update;
    else
        result.$unset._update = "";
    if (source._deleted) {
        result.$set._deleted = source._deleted;
        result.$set.status = "deleted";
    }
    else
        result.$unset._deleted = "";
    if (Object.keys(result.$unset).length === 0)
        delete result.$unset;
    return result;
}
async function runUpdateDbObjects() {
    //const dbCollections = await db
    const mgObjects = mg.model('dbobjects', new mg.Schema({}, { strict: false }), 'dbobjects');
    const dbObjects = (await mgObjects.find({})).map((i) => i._doc);
    await Promise.all(collections.map(async (collection) => {
        const model = mg.model(collection, new mg.Schema({}, { strict: false }), collection);
        if (model) {
            const docs = (await model.find({})).map((i) => i._doc);
            const inserted = [];
            const updated = [];
            docs.forEach(doc => {
...
exports.runUpdateDbObjects = runUpdateDbObjects;
const cleanObjectsFromCreateUpdate = async () => {
    await Promise.all(collections.map(async (collection) => {
        const model = mg.model(collection, new mg.Schema({}, { strict: false }), collection);
        if (model) {
...
exports.cleanObjectsFromCreateUpdate = cleanObjectsFromCreateUpdate;
//# sourceMappingURL=clone-all-entities-to-objects-with-date.js.map