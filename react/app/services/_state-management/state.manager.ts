...
class _StorageManager {
	storages: { [key: string]: Storage<any> } = {};
	createStorage<S extends object, StorageType extends Storage<S>>(
		storageName: string,
		baseClass: Newable<StorageType>,
		initialState: S
	) {
		const res = new baseClass(initialState);
		this.storages[storageName] = res;
		return res as StorageType;
	}

	get(storageName: string) { return this.storages[storageName]; }
}
